-- Project Afyon 0.34 — local chance outcomes, server persistence only
-- The browser decides all chance-game randomness. This RPC only applies the
-- submitted payout to the authenticated wallet and records history.
-- p_client_id makes retries idempotent.
...
revoke all on function public.settle_local_chance_game(uuid,text,bigint,text,jsonb,numeric) from public;
revoke all on function public.settle_local_chance_game(uuid,text,bigint,text,jsonb,numeric) from anon;
grant execute on function public.settle_local_chance_game(uuid,text,bigint,text,jsonb,numeric) to authenticated;

-- New local flow no longer uses server-side penalty sessions.
delete from private.chance_sessions where game_key='penalty_series' and status='active';
