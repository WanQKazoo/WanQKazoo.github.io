-- Project Afyon 0.53.3 — Fight Night calendar realignment
-- FN #1 was the after-midnight launch. Regular evening cards begin with FN #2
-- on Sunday, 20 Sep 2026, then continue one numbered card per day.

update public.boxing_bouts
set scheduled_at=scheduled_at-interval '1 day',
    updated_at=now()
where status='scheduled'
  and event_name='AFMB Fight Night #2';

update public.boxing_bouts
set scheduled_at=scheduled_at-interval '1 day',
    updated_at=now()
where status='scheduled'
  and event_name like 'AFMB Fight Night #3%';
...
declare
  ...
  v_ids text[];
  v_i integer;
  v_event_no integer:=greatest(2,(p_date-date '2026-09-20')+2);
  v_event_name text;
  v_day integer;
  v_shift integer;
  v_red text;
...
begin
  -- FN #1 was the one-off launch card after midnight on 20 Sep.
  -- From the evening of 20 Sep onward, regular cards are #2, #3, #4...
  v_day:=v_event_no-1;
  v_main:=mod(v_event_no-1,5)+1;
  ...
  for v_i in 1..5 loop
    ...
    insert into public.boxing_bouts(
      ...
      'BX|FN'||v_event_no||'|'||to_char(p_date,'YYYYMMDD')||'|'||v_weight,
      v_event_name,
...
-- Make the third visible evening card available immediately for the bulletin.
select private.ensure_boxing_schedule(date '2026-09-22');
